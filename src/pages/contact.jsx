...
export default function Contact() {
    ...
    return (
        <div className="flex flex-col md:flex-row mt-3 max-w-8xl w-11/12 sm:w-10/12 mx-auto" data-aos="fade-right">
            <div className="flex-1">
                ...
                <h3 className="font-sans text-gray-500 text-sm mt-1">Anyone who sends a message for no reason will be banned.</h3>
                ...
                <form className="mt-4" action="https://formspree.io/f/mjvdkjoq" method="POST" onSubmit={async (event) => {
                    event.preventDefault()

                    setStatus({type: "WAIT"})

                    let sendedData = await fetch("https://formspree.io/f/mjvdkjoq", {
                        method: "POST",
                        body: JSON.stringify({
                    ...
                    <input 
                        ...
                        className="mt-2 outline-none p-4 w-full sm:w-7/12 bg-primary-100 rounded-lg font-sans text-base text-white"
                    />
                    ...
                    <input 
                        ...
                        type="email" 
                        minLength="10" 
                        maxLength="45" // Maksimum 20 karakter
                        name="email" 
                        className="mt-2 outline-none p-4 w-full sm:w-7/12 bg-primary-100 rounded-lg font-sans text-base text-white"
                    />
 ...
 maxLength="2000"
                        ...
                        ref={message} 
                        name="message" 
                        className="resize-none min-h-[9rem] mt-2 outline-none p-4 w-full sm:w-7/12 bg-primary-100 rounded-lg font-sans text-base text-white"
                    />
                    ...
                    <button 
                        type="submit" 
                        className={`py-3 px-4 bg-primary text-white font-sans text-base rounded-lg w-full sm:w-auto flex items-center gap-2 justify-center sm:justify-start${(status.type !== "NOTHING" && " opacity-30 cursor-not-allowed")}`} 
                        disabled={(status.type === "NOTHING") ? false : true}
                    >
                        {{
                            "NOTHING": (
            ...
            <div className="flex-none ml-4">
                <iframe src="https://discord.com/widget?id=971823165351280685&theme=dark" width="350" height="500" allowtransparency="true" frameborder="0" sandbox="allow-popups allow-popups-to-escape-sandbox allow-same-origin allow-scripts"></iframe>
            </div>
        </div>
    )
}
